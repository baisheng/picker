...
export class TermService {
  ...
  // private
  // private wrapTerm(term: Term) {
  // slugify
  // }
  public batchCreateTerm(terms: Term[]): Promise<Term[]> {
    return this.connection.getRepository(Term)
      .save(terms)
      .then(result => {
        return result;
      });
   ...
   * @param newTerm
   */
  public create(newTerm: Term): Promise<Term> {
    return this.connection.getRepository(Term)
      .save(newTerm)
      .then(entity => {
        return entity;
   ...
   * @param newTaxonomy
   */
  public createTaxonomy(newTaxonomy: TermTaxonomy): Promise<TermTaxonomy> {
    return this.connection.getRepository(TermTaxonomy)
      .save(newTaxonomy)
      .then(entity => {
        return entity;
      });
  ...
  async loadAllTerms(flag?: boolean): Promise<any> {
    ...
    if (_.isEmpty(ret)) {
      ...
      if (_.isEmpty(data)) {
        // throwError
        throw new CustomError({ message: '数据库中分类内容为空。' }, 404);
      }
      // 以下处理 meta 信息
      // IN 查询处理 ids
      const ids = [];
   ...
   * @param taxonomyId
   */
  async relationships(objectId: ID, taxonomyIds: ID[]) {
    const relations = [];
    for (const taxonomyId of taxonomyIds) {
      ...
      //   taxonomyId,
      // });
      await this.relationshipsSingle(objectId, taxonomyId);
    }
