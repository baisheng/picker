import { HeroesService } from '@app/modules/users/test.service';
...
@Controller('heroes')
@Feature('heroes')
export class HeroesController implements CrudController<User> {
  ...
  @Action('Read-One')
  async oneself(@ParsedRequest() req: CrudRequest) {
    let user = await this.base.getOneBase(req);
    // const user = await this.service.findByIdentifier(request.user.identifier);
    // 请求统计信息
    if (user) {
      user = formatOneMeta(user, {cleanMeta: true});
      return user;
    }
    return null;
  }
