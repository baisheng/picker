...
import { filter, isEmpty, findLast, flattenDeep, intersectionBy, groupBy, has } from 'lodash';
...
import { TermMeta } from '@app/modules/terms/term-meta.entity';
...
export class TagService {
  ...
  async findCategoriesByObject(objectId: ID) {
    ...
    const taxonomy = new TermTaxonomy({
      taxonomy: 'category',
    });
    // 从缓存中提取所有 term
    // const allTerms: Term[] = await this.cacheService.get(CACHE_KEY.TERMS);
    const allTerms: Term[] = await this.cacheService.get<Term[]>(CACHE_KEY.TERMS);
    ...
    taxonomies.forEach(item => {
      terms.push(filter(allTerms, {
        taxonomyId: item.taxonomyId,
        taxonomy,
      }));
    });
    return flattenDeep(terms);
  }

  /**
   * 处理 post-format 格式对象
   */
  async formatTermForObject(objectId: ID): Promise<Term | any> {
    // const allTerms: Term[] = await this.cacheService.get(CACHE_KEY.TERMS);
    const allTerms: Term[] = await this.cacheService.get<Term[]>(CACHE_KEY.TERMS);
    ...
    const taxonomies = await this.connection.manager
      ...
      .getRawMany();
    // 交叉比对类别 ID 并合并
    const curPostHasTerms = intersectionBy(allTerms, taxonomies, 'termId');
    // console.log(curPostHasTerms);
    // 处理包含 post-format 格式
    const postFormat = findLast(curPostHasTerms, (o) => {
      // console.log(o.slug);
  ...
  async getFromCategory(categorySlug: string, status?: string, querys?: any) {
    ...
    if (!isEmpty(objIds)) {
      ...
      data.forEach(item => {
        item.metas = filter(metaData, { id: item.id });
      });
    }
    return data;
  }
  ...
  async loadAllTerms(flag?: boolean): Promise<any> {
    ...
    if (isEmpty(ret)) {
      ...
      const metaList = await this.connection.manager
        ...
        .getRawMany();
      // 数据分组
      const metaGroup = groupBy(metaList, 'termId');
  ...
  private formatMeta(list: any[]) {
    ...
    for (const item of list) {
      item.meta = {};
      if (has(item, 'metas') && item.metas.length > 0) {
