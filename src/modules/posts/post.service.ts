...
export class PostService {
  ...
  async findById(id: ID): Promise<PostEntity> {
    const data: PostEntity = await this.connection.getRepository(PostEntity)
      .findOneOrFail({
        where: {
          id,
        },
   ...
   * @param newPost
   */
  public create(newPost: PostEntity): Promise<PostEntity> {
    // Object.assign(newPost, {meta: })
    return this.connection.getRepository(PostEntity)
      .save(newPost)
      .then(entity => {
        return entity;
      });
